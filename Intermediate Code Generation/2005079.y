...
func_definition : 
	...
	{ 
		int tempStackOffset = $4->getListItem().size() + 1;
		for (auto item: $4->getListItem())
		{
...
var_declaration : 
	type_specifier declaration_list SEMICOLON
	{
		...
		$$->setGlobal($2->getGlobal());
		$$->setStackOffset($2->getStackOffsetVal());
	}
...
statement : 
	...
	| PRINTLN LPAREN ID RPAREN SEMICOLON
	{
		...
		if (temp)
		{
			$3->setGlobal(temp->getGlobal());
			$3->setStackOffset(temp->getStackOffsetVal());
			$3->setValue(temp->getValue());
		}
	}
...
variable : 
	ID
	{
		...
		else
		{
			...
			$$->setStackOffset(temp->getStackOffsetVal());
			$$->setGlobal(temp->getGlobal());
...
factor	: 
	variable 
	{
		...
		if(temp)
		{
			...
			$1->setValue(temp->getValue());
			$$->setValue($1->getValue());
			$1->setStackOffset(temp->getStackOffsetVal());
			$$->setStackOffset(temp->getStackOffsetVal());
		}
	}
