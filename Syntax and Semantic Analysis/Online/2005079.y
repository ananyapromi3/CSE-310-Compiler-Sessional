...
%token<symbolInfo> LOOP IF ELSE FOR WHILE DO BREAK CONTINUE RETURN VOID SWITCH CASE DEFAULT PRINTLN ADDOP MULOP INCOP RELOP ASSIGNOP LOGICOP BITOP NOT INT CHAR FLOAT DOUBLE ID NEWLINE LPAREN RPAREN LCURL RCURL LSQUARE RSQUARE COMMA SEMICOLON DECOP CONST_CHAR CONST_FLOAT CONST_INT
...
statement : 
	...
	| LOOP statement WHILE LPAREN expression RPAREN SEMICOLON
	{
		logout << "statement : LOOP statement WHILE LPAREN expression RPAREN SEMICOLON\n";	
		$$ = new SymbolInfo("", "statement", $1->getLineStart(), $7->getLineEnd());
		$$->setRule("statement : LOOP statement WHILE LPAREN expression RPAREN SEMICOLON");
		$$->addChild($1);
		$$->addChild($2);
		$$->addChild($3);
		$$->addChild($4);
